...
2.1 交易币买入例子：  
以市价下单 买入 10个LTC为例，用户可买为11个，此时 10 < 11，挂单成功。当LTC-USDT的市价为200，用户被锁定余额为3000
USDT，200*10 < 3000，最终成交10个LTC； 若市场波动过大，LTC-
USDT的市价为400，此时400*10>3000，当用户被锁定的余额不够买入下单指定的交易货币数量时，系統使用用户被锁定的最大余额3000
...
期权的张币转换时，可不填。  
unit | String | 否 | 币的单位，coin: 币，usdt: usdt  
仅适用于交割和永续合约的U本位合约  
...
`5`：交易服务；`6`：大宗交易；`99`：其他（如：停止部分产品交易）  
...
获取标记价格的K线数据，推送频率最快是间隔1分钟推送一次数据。
...
50017 | 200 | 当前仓位处于ADL冻结中，无法进行相关操作  
50018 | 200 | {0}币种处于ADL冻结中，无法进行相关操作  
50019 | 200 | 当前账户处于ADL冻结中，无法进行相关操作  
50020 | 200 | 当前仓位处于强平冻结中，无法进行相关操作  
50021 | 200 | {0}币种处于强平冻结中，无法进行相关操作  
50022 | 200 | 当前账户处于强平冻结中，无法进行相关操作  
50023 | 200 | 资金费冻结，无法进行相关操作  
...
51007 | 200 | 委托失败，委托数量不可小于 1 张（用户下单数量不足 1 张时）  
51008 | 200 | 委托失败，账户 {0} 可用余额不足  
51008 | 200 | 委托失败，账户 {0} 可用保证金不足  
51008 | 200 | 委托失败，账户 {0} 可用余额不足，且未开启自动借币  
51008 | 200 | 委托失败，账户 {0} 可用保证金不足，且未开启自动借币  
51008 | 200 | 委托失败，因为 {0} 剩余的档位限额不足，导致可借不足（需借：{1}， 剩余额度{2}，限额 {3}，已用额度{4}）  
51008 | 200 | 委托失败，因为 {0} 剩余的母账户限额不足，导致可借不足（需借 {1}，剩余额度 {2}，限额 {3}，已用额度 {4}）  
51008 | 200 | 委托失败，因为 {0} 剩余的币对限额不足，导致可借不足  
51008 | 200 | 委托失败，因为 {0} 剩余的借贷池限额不足，导致可借不足  
51008 | 200 | 委托失败，账户资产不足，美金层面有效保证金小于 IMR  
51008 | 200 | 委托失败，账户资产不足，且 Delta 校验不通过  
...
51147 | 200 | 交易账户资产总价值需要大于5万美元来交易期权  
...
51295 | 200 | PM账户不支持ordType为{0}的策略委托单  
...
58203 | 200 | 请先添加提现地址  
58204 | 200 | 提现冻结  
58205 | 200 | 提现金额大于单笔提现最大金额（单笔提现最大金额提现接口，提现金额输入有误）  
58206 | 200 | 提现金额小于最小提现金额（最小提现金额提现接口，提现金额输入有误）  
...
58354 | 200 | 单日达到生成invoice10000个的上限  
58355 | 200 | 用户没有使用此API接口的权限  
...
59005 | 200 | 逐仓自主划转保证金模式，初次划入仓位的资产价值需大于10000USDT  
...
70006 | 200 | 不满足最小资产要求  
70007 | 200 | 该产品类型 {0} 的标的指数 {0} 不存在  
70008 | 200 | MMP状态下操作失败。冻结时间为 {0} 秒  
70009 | 200 | Data数组必须至少含有一个有效元素  
70011 | 200 | 产品类型 {0} 存在重复设置  
70012 | 200 | 同一个instType{1}下的instFamily/instId{0} 存在重复设置  
70013 | 200 | 交易产品设置中需选择至少一个交易品种  
70014 | 200 | 不允许对所有产品类别设置includeAll=True.  
70100 | 200 | 组合交易中的产品ID重复  
70101 | 200 | clRfqId重复  
70102 | 200 | 未指定对手方  
70103 | 200 | 无效的对手方  
70105 | 200 | 非全现货的RFQ总价值应该大于最小名义值{0}  
70106 | 200 | 下单数量小于最小交易数量  
70107 | 200 | 对手方的数量不能超过最大值  
70108 | 200 | 全现货的RFQ总价值应该大于最小名义值{spotMinNotional}  
70109 | 200 | 所选产品无有效对手方  
70200 | 200 | 不能取消处于{0}状态的询价单  
70203 | 200 | 取消失败，由于询价单数量超过限制数量{0}  
70207 | 200 | 取消失败，由于您没有询价挂单  
70208 | 200 | 取消失败，由于服务暂时不可用，请稍后重试  
70301 | 200 | clQuoteId重复  
70303 | 200 | 不能对处于{0}状态的询价单报价  
70304 | 200 | 价格应该是下单价格精度的整数倍  
70305 | 200 | 买入价格不能高于报价  
70306 | 200 | 报价的组合交易没有匹配{0}的组合交易  
70307 | 200 | 数量应该是下单数量精度的整数倍  
70308 | 200 | 不允许对自己的询价单报价  
70309 | 200 | 不允许对相同询价单进行同一方向的报价  
70310 | 200 | instId {0} 报价不可以超过你预设的价格限制  
70400 | 200 | 不能取消处于{0}状态的报价单  
70408 | 200 | 取消失败，由于报价单数量超过限制数量{0}  
70409 | 200 | 取消失败，由于您没有报价挂单  
70501 | 200 | 询价单{0}没有被{1}报价  
70502 | 200 | 组合交易没有匹配{0}的组合交易  
70503 | 200 | 执行腿的价值总和小于大宗交易的最小名义值  
70504 | 200 | 执行失败，因为询价单的状态是{0}  
