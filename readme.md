[![](../static/img/logo.png?_st=20200417184032)](/)
...
(3) 签名前字符串是否符合标准格式，所有要素的顺序要保持一致，可以复制如下示例跟您的签名前字符串进行比对：
    ...
    POST示例：2020-03-28T12:21:58.152ZPOST/api/account/v3/transfer{"currency" : "eos","amount":0.1,"from":6,"to":1,"instrument_id":"eos-usdt"}
...
通过"提币手续费"、"获取充值地址"这两个接口的返回值进行判断，同时请求参数 currency 需传入 usdt
...
（1）未添加心跳，WebSocket连接需在接收到服务端发送的Ping信息后回复Pong，保证连接的稳定。
...
最多获取1440条，更多的获取不到，如需要更多的，需要在本地保存一下；另外返回的是数据原始信息，API没有指标数据相关的接口；
...
**3. 为什么获取成交明细接口1笔交易会返回2条数据？**

因为这个接口一笔成交会返回2条数据，一个以计价货币计算的，一个是以交易货币计算的。

比如BTC-USDT，买入1BTC，一笔是买入1BTC，同时会有一笔是卖出等值的USDT。

**4. "杠杆配置信息"返回的当前最大可借，是考虑了所有因素的么？**
...
before | String | 否 | 请求此id之后（更新的数据）的分页内容，传的值为对应接口的`ledger_id`等 |  
limit | String | 否 | 分页返回的结果集数量，最大为100，不填默认返回100条  
type | String | 否 | 1.充值 2.提现 7.买入 8.卖出 18.交割合约转 19.转出至交割合约 20.转入子账户21.转出至子账户
...
币种不存在 | 30031 | 400  
当前的借币请求系统正在处理中，请稍后重试 | 30032 | 400  
